...
def test(model, test_loader, save=False):
    ...
    loss_fn = nn.MSELoss()
    with torch.no_grad():
        for i, (data, template,_,_) in enumerate(test_loader):
            data = data.cuda(non_blocking=True)
            template = template.cuda(non_blocking=True)
            out, _, _ = model(data, template)
            #out = out.squeeze()
            print(loss_fn(out[0],data[0]))
            if save:
                np.save(os.path.join(save_folder, str(i) +"pre.npy"), out.detach().cpu().numpy())
                np.save(os.path.join(save_folder, str(i) +"gt.npy"), data.detach().cpu().numpy())
