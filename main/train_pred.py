...
def train(train_loader, model, loss_fn, optimizer, epoch, cfg):
    ...
    for i, ( data, template, name,onehot) in enumerate(train_loader):
        # pdb.set_trace()
        ####################
        ...
        onehot = onehot.cuda(cfg.gpu, non_blocking=True)
    
        loss, _ = model(template, data, onehot, criterion=loss_fn)
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
...
def validate(val_loader, model, loss_fn, cfg):
    ...
    with torch.no_grad():
        for i, (vertice, template, one_hot, _) in enumerate(val_loader):
